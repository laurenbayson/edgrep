  ...
  do {
    ...
    if ('0'<=c && c<='9')
     ...
     if (!opcnt)  { a = zero; }  a += sign*getnum();
      ...
      default:
        if (nextopand == opcnt) 
        {  a += sign;  if (a < zero || dol < a)  { continue; } /* error(Q); */ }
        if (c!='+' && c!='-' && c!='^') 
        {  peekc = c;  
        if (opcnt==0) 
        { a = 0; }  
        ...
        sign = 1;  
        if (c!='+') 
        { sign = -sign; }  nextopand = ++opcnt;  continue;
    }
    sign = 1;  opcnt++;
  } while (zero<=a && a<=dol);
    ...
    switch (c) {
      ...
      case '[':  *ep++ = CCL;  *ep++ = 0;  cclcnt = 1;  if ((c=getchr()) == '^') {  c = getchr();  ep[-2] = NCCL; }
        do {
          ...
          *ep++ = c;  
          cclcnt++;  /*if (ep >= &expbuf[ESIZE]) { goto cerror; }*/
        } while ((c = getchr()) != ']');
...
{  int c;  
  ...
  { while ((c = getchr()) != '\n' && c != EOF) { } }
  if (io > 0) 
  { close(io);  io = -1; }  longjmp(savej, 1);
}
int execute(unsigned int *addr) 
{  char *p1, *p2 = expbuf;  
   int c;
  for (c = 0; c < NBRA; c++) 
  {  braslist[c] = 0;  braelist[c] = 0;  }
  if (addr == (unsigned *)0) {
    if (*p2 == CCIRC) 
    { return(0); }  
    p1 = loc2; } 
  ...
  if (*p2 == CCIRC) 
  {  loc1 = p1;  return(advance(p1, p2+1)); }
