...
void search_file(const char* filename, const char* searchfor) {
  printf("\n");  grepline();  grepline();  printf("processing %s...\n", filename);  grepline();
  readfile(filename);
  search(searchfor);
}

void printcommand(void) {  int c;  
char lastsep;
  for (;;) {  unsigned int* a1;
    if (pflag) { pflag = 0;  
    addr1 = addr2 = dot;  
...
int getchr(void) {  char c;
  ...
  if (globp) {  if ((lastc = *globp++) != 0) { return(lastc); }  globp = 0;  
  return(EOF);  }
...
int getnum(void) { int r = 0, c;
  while ((c = getchr())>='0' && c <= '9') { r = r * 10 + c - '0'; }  peekc = c;  
  return (r);
}
...
void global(int k) {  char *gp;  int c;  unsigned int *a1;  char globuf[GBSIZE];
  ...
  *gp++ = 0;
  for (a1 = zero; 
  a1 <= dol; a1++) {  *a1 &= ~01;  if (a1>=addr1 && a1<=addr2 && execute(a1)==k) { *a1 |= 01; } }
  for (a1 = zero; 
  a1 <= dol; a1++) {
    if (*a1 & 01) {  *a1 &= ~01;  
    dot = a1;  
...
nonzero();
  ...
  putd();  
  putchr_('\t');  }  puts_(getline_blk(*a1++));  } while (a1 <= addr2);
  dot = addr2;  
  listf = 0;  
  listn = 0;  
  pflag = 0;
